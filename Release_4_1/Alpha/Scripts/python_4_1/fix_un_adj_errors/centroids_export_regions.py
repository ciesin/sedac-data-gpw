#Jane Mills
#11/29/2017
#Export regional/global centroids
...
inGDB = r'\\Dataserver0\gpw\GPW4\Release_4_1\Alpha\Gridding\global\centroids_data\merged_data.gdb'
...
for region in regionList:
    print region
    inFC = os.path.join(inGDB,region+"_centroids")
    outGDB = os.path.join(outFolder,region.lower()+"_centroids.gdb")
    outFC = os.path.join(outGDB,region.lower()+'_centroids')

    arcpy.CreateFileGDB_management(outFolder, region.lower()+"_centroids.gdb")

    #set up csv
    outcsv = os.path.join(outFolder,region.lower()+'_centroids.csv')
    csvMem = csv.writer(open(outcsv,"wb"))
    csvMem.writerow(headers)

    #Add centroids to gdb and csv
    ...
    #Save as gpkg
    outFile = os.path.join(outFolder,region+'_centroids.gpkg')
    inDataSource = inDriver.Open(outFC,0)
    outData = outDriver.CreateDataSource(outFile)
    arcpy.FeatureClassToFeatureClass_conversion(outFC,outFile,region.lower()+"_centroids")
    print "gpkg"
