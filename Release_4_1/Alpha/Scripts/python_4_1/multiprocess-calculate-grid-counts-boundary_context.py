...
def grid(fishnet, gridFields, gdb):
    ...
    outWS = inWS.replace(os.path.basename(inWS),'boundary_context_tifs')
...
def calculateGridCounts(gdb):
    ...
    try:
        ...
        # grab intersectTable
        intersectTables = arcpy.ListTables("*boundary_context_intersect_estimates_table")
        if len(intersectTables)==0:
            return
        intersectTable = intersectTables[0]
        fishnet = arcpy.ListFeatureClasses("*_fishnet")[0]
        finalFishnet = fishnet + "_boundary_context_processed"
