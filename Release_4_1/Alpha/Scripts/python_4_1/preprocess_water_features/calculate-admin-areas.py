...
def calculateAdminAreas(inFC):
    ...
    iso = rootName[:3]
    waterMask = r'D:\gpw\release_4_1\water\water_outputs\water_masks_v2' + os.sep + iso + "_water_mask.shp"
    if not arcpy.Exists(waterMask):
        hasWater = 0
    else:
        hasWater = 1
    try:
        outFolder = r'D:\gpw\release_4_1\water\water_outputs\water_masks_v2'
...
##                            return values[tmpid]
        ...
        features2Check = r'D:\gpw\release_4_1\water\boundary_greater_than_85pct_water_v2' + os.sep + iso + ".shp"
        threshold = 85
...
def main():
    ...
    isos = ['aut','bgr','bih','bra','che','cod','deu','egy','fra','hrv','hun','irl','kos',
            'moz','mys','nam','pri','prt','rou','srb','svn','twn','ven','zaf','zmb','zwe']
    for fc in fcs:
        if fc[:3] in isos:
