...
def update_rasters(raster):
    scratchDir = r"D:\gpw\release_4_1\scratch" + os.sep + os.path.basename(raster.lower())[:-8]    
    ...
    if os.path.basename(raster.lower())[:-8] == "can" or os.path.basename(raster.lower())[:-8] == "chn" or os.path.basename(raster.lower())[:-8] == "usa" or os.path.basename(raster.lower())[:-8] == "rus":
        ...
        # create output gdb
        outGDB = r"D:\gpw\release_4_1\scratch" + os.sep + os.path.basename(raster.lower())[:-8] + ".gdb"
        if not arcpy.Exists(outGDB):
            arcpy.CreateFileGDB_management(r"D:\gpw\release_4_1\scratch",os.path.basename(raster.lower())[:-8])
        else:
            pass
        # convert to polygons
...
def main():
    ...
    rasters = arcpy.ListRasters('black*ids.tif*')
