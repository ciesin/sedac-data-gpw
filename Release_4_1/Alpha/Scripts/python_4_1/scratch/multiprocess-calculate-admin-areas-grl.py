...
def calculateAdminAreas(inFC):
    ...
    rootName = os.path.basename(inFC.replace("_gridding",""))                          
...
##    waterMask = r'F:\gpw\release_4_1\water\water_outputs\water_masks' + os.sep + iso + "_water_mask.shp"
    ...
    try:
        ...
        outGDB = outFolder + os.sep + rootName + ".gdb"
...
##        arcpy.CalculateField_management(inFCG,"ISO",'"' + iso.upper() + '"',"PYTHON")
        ...
        if not arcpy.Exists(waterFeatures):
            hasWater = 0
        else:
            hasWater = 1
...
def main():
    ...
    gdbs = arcpy.ListWorkspaces("grl_*")
    fcList = []
    for gdb in gdbs:
        arcpy.env.workspace=gdb
        fc = arcpy.ListFeatureClasses("*gridding")[0]
        fcList.append(os.path.join(gdb,fc))
