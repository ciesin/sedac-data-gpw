...
def calculateAdminDensities(gdb):
    ...
    try:
        ...
        adminBoundaries = arcpy.ListFeatureClasses("*_boundary_context")
        if len(adminBoundaries)==1:
            adminBoundaries = arcpy.ListFeatureClasses("*_boundary_context")[0]
        else:
            return
        # define initial list of searchFields
        boundarySearchFields = ["UBID","ADMINAREA"]
        searchFields = ["UBID","ADMINAREA"]
        updateFields = ["UBID","ADMINAREA"]
        ...
        arcpy.AddField_management(inMemEstimates,"ADMINAREA","DOUBLE")
        # create list of variables
        eVariables = arcpy.ListFields(inMemEstimates,"E_*DSM")
        uneVariables = arcpy.ListFields(inMemEstimates,"UNE_*DSM")
        variables = eVariables + uneVariables
        for variable in variables:
            newField = variable.name
##            # add the field
...
##                arcpy.AddField_management(inMemEstimates,newField,"DOUBLE")
            ...
            searchFields.append(newField.replace("_DSM",""))
...
##                    return (searchFields, row)
                        ...
                        for field in updateFields:
                            ...
                            if i < 1:
                                ...
                                try:
                                    ...
                                    return ('error with prop calc',estimates[ubid],area)
                                i = i + 1                            
        ...
        try:
            estimatesLyr = iso + 'lyr'
            arcpy.MakeTableView_management(inMemEstimates, estimatesLyr, "ADMINAREA <> 0",)
            estimatesFile = os.path.join(gdb,os.path.basename(estimatesIn)+"_boundary_context")
            arcpy.CopyRows_management(estimatesLyr,estimatesFile)
        except:
