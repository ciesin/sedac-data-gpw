...
USAoutFC = os.path.join(outGDB,'usa_centroids')
...
arcpy.CopyFeatures_management(template,USAoutFC)
arcpy.DeleteField_management(USAoutFC,'UBID')
...
##                USAcsvMem.writerow(row)
        arcpy.Append_management(fcPath,USAoutFC,"NO_TEST")
...
inDataSource = inDriver.Open(USAoutFC,0)
outData = outDriver.CreateDataSource(USAoutFile)
arcpy.FeatureClassToFeatureClass_conversion(USAoutFC,USAoutFile,"usa_centroids")
