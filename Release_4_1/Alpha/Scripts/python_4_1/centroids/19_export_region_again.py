#Jane Mills
#3/29/2017
#Export everything to csv and gpkg
...
root = r'\\Dataserver0\gpw\GPW4\Release_4_1\Alpha\Gridding\centroids\global'

#regionList = ['africa','asia','europe',]
regionList = ['north_america','oceania','south_america']

for region in regionList:
    print region

    inFC = os.path.join(root,region+"_centroids.gdb",region+"_centroids")
    ...
    outcsv = os.path.join(root,region+'_centroids.csv')
    ...
    #Save as gpkg
    outFile = os.path.join(root,region + '_centroids.gpkg')
    inDataSource = inDriver.Open(inFC,0)
    outData = outDriver.CreateDataSource(outFile)
    arcpy.FeatureClassToFeatureClass_conversion(inFC,outFile,region+"_centroids")
