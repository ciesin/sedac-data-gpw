...
inGDB = r'\\Dataserver0\gpw\GPW4\Release_4_1\Alpha\Gridding\global\centroids\merged_data.gdb'
csvFolder = os.path.join(root,'csv')
gpkgFolder = os.path.join(root,'gpkg')

inFC = os.path.join(inGDB,'global_centroids')
USAinFC = os.path.join(inGDB,'usa_centroids')
...
##
##USAoutcsv = os.path.join(csvFolder,'usa_centroids.csv')
##USAcsvMem = csv.writer(open(USAoutcsv,"wb"))
##USAcsvMem.writerow(headers)
##
##outcsv = os.path.join(csvFolder,'global_centroids.csv')
##csvMem = csv.writer(open(outcsv,"wb"))
##csvMem.writerow(headers)
##
###Add centroids to csv
##with arcpy.da.SearchCursor(USAinFC,headers) as cursor:
##    for row in cursor:
##        USAcsvMem.writerow(row)
##print "completed usa csv"
##
##with arcpy.da.SearchCursor(inFC,headers) as cursor:
##    for row in cursor:
##        csvMem.writerow(row)
##print "completed global csv"

#Save as gpkg
outFile = os.path.join(gpkgFolder,'global_centroids.gpkg')
inDataSource = inDriver.Open(inFC,0)
outData = outDriver.CreateDataSource(outFile)
arcpy.FeatureClassToFeatureClass_conversion(inFC,outFile,"global_centroids")
print "completed global geopackage"

USAoutFile = os.path.join(gpkgFolder,'usa_centroids.gpkg')
inDataSource = inDriver.Open(USAinFC,0)
outData = outDriver.CreateDataSource(USAoutFile)
arcpy.FeatureClassToFeatureClass_conversion(USAinFC,USAoutFile,"usa_centroids")
print "completed usa geopackage"
