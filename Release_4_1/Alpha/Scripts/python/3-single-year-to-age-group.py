...
def main():
    ...
    for ws in workspaces:
        ...
        tbls.sort()
        tbls_temp = [os.path.join(ws, tbl) for tbl in tbls]
        for tblt in tbls_temp:
            tblList.append(tblt)    
    ...
    for tbl in tblList:
        print tbl
        # determine if the data already contains age groups

        ## MAYBE SHOULD INSTEAD READ THIS FROM THE VARIABLE SUMMARIES
        # determine the highest age in the table
        flds = arcpy.ListFields(tbl,"*BT")
        for fld in flds:
            print fld.name
        break
