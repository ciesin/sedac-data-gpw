...
#########evaluate POPCONTEXT field and extract rows if needed##########
# check if inRawTable has POPCONTEXT field
if custom.checkForField(inRawTable,'POPCONTEXT')==1:
    # if yes then create a condition to check if there are any
    # non-zero popcontext rows and use custom.checkTableCondition to extract tables
    popContextClause = 'POPCONTEXT>0'
    popContextView = os.path.basename(inRawTable) + '_popcontext'
    outPopContextTbl = inRawTable + '_popcontext'
    inputPopClause = 'POPCONTEXT=0'
    inputPopView = os.path.basename(inRawTable) + '_input_temp'
    outPop1Tbl = rootName + '_input_population_v01'
    print custom.checkTableCondition(inRawTable,popContextClause,popContextView,outPopContextTbl,inputPopClause,inputPopView,outPop1Tbl)[1]
    validationReports.append((1,'Check POPCONTEXT Table ',outPopContextTbl))
    
# if the popcontext field is missing, then we might need to check, so add message
else:
    print 'POPCONTEXT Field is not present in ' + inRawTable + '. Should it be?'
    arcpy.AddMessage('POPCONTEXT Field is not present in ' + inRawTable + '. Should it be?')
    validationReports.append((1,'POPCONTEXT Field is not present in ' + os.path.basename(inRawTable) + '. Should it be? Double check',None))
    # however, it might be valid for this field to be missing, so create outPop1
    outPop1Tbl = rootName + '_input_population_v0'
    try:
        arcpy.CopyRows_management(inRawTable,outPop1Tbl)
        print 'Created ' + outPop1Tbl
        arcpy.AddMessage('Created ' + outPop1Tbl)
    except:
        print arcpy.GetMessages()
...
if custom.checkForField(outPop1Tbl,'ATOTPOPBT')==1:
    ...
    # run checkTableCondition, if it fails set outPop2Tbl to outPop1Tbl
    # failure means that there aren't any negative value rows, no need to process further
    if int(custom.checkTableCondition(outPop1Tbl,negClause,negValueView,outNegTbl,posClause,posValueView,outPop2Tbl)[0])==0:
        outPop2Tbl = outPop1Tbl
else:
    print 'ATOTPOPBT Field is not present in ' + outPop1Tbl
    arcpy.AddMessage('ATOTPOPBT Field is not present in ' + outPop1Tbl)
    validationReports.append((1,'ATOTPOPBT Field is not present. Check inputs',None))

# check if ISO exists
if custom.checkForField(outPop2Tbl,'ISO')==1:
    # if yes, then pass
    pass
else:
    # otherwise add and calculate
    try:
        arcpy.AddField_management(outPop2Tbl,'ISO','TEXT','#','#',5)
        arcpy.CalculateField_management(outPop2Tbl,'ISO','"'+iso.upper()+'"','PYTHON')
...
if not custom.checkForField(outPop2Tbl,'RPOPYEAR')==1:
    try:
        # perform calculation
        arcpy.AddField_management(outPop2Tbl,'RPOPYEAR','SHORT')
        arcpy.CalculateField_management(outPop2Tbl,'RPOPYEAR','!'+yearField+'!','PYTHON')
        arcpy.DeleteField_management(outPop2Tbl,yearField)
...
validationResults = custom.validateSchema(outPop2Tbl,schemaTable)
...
for validationResult in validationResults:
    ...
    if validationResult[0]==2:
        ...
        try:
            arcpy.AddField_management(outPop2Tbl,tmpField,validationType)
            arcpy.CalculateField_management(outPop2Tbl,tmpField,tmpCalc,"PYTHON")
            arcpy.DeleteField_management(outPop2Tbl,validationField)
            arcpy.AddField_management(outPop2Tbl,validationField,validationType)
            arcpy.CalculateField_management(outPop2Tbl,validationField,validationCalc,"PYTHON")
            arcpy.DeleteField_management(outPop2Tbl,tmpField)
...
try:
    ...
    print arcpy.GetMessages()
try:
    arcpy.Append_management(outPop2Tbl,outPopTable,"NO_TEST")
except:
    print arcpy.GetMessages() 
...
if int(arcpy.GetCount_management(arcpy.MakeTableView_management(outPopTable,nullUBIDView,nullExp))[0])>0:
    ...
    validationReports.append((0,"There are null UBIDs, Check and rerun.",nullUBIDTbl))
