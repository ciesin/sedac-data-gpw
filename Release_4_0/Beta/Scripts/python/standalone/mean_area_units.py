    ...
    try:
        ...
        with arcpy.da.UpdateCursor(inFish, targFields) as recs:
            for rec in recs:
                ...
                recs.updateRow(rec)
        del rec, recs
        gdb = r'Z:\GPW4\Beta\Gridding\rasters\can\tiles' + os.sep + iso + ".gdb"
...
##        meanUnitArea.save(meanUnits)    
        # success
        return "Computed Mean Admin Unit Area Inputs for " + iso.upper() + ": " + str(datetime.datetime.now()-startTime)
    except:
...
##    print meanAdminArea(gdb)
    ...
    workspaces = [r'Z:\GPW4\Beta\Gridding\country\fishnets_and_clipped_water\can']#usaSpace,tiledSpace
