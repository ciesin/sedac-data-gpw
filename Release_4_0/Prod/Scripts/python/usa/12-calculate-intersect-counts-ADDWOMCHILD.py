...
def calculateIntersectCounts(gdb):
    ...
    try:
        ...
        for fishGDB in fishList:   
            ...
            if arcpy.Exists(intersectTable):
                # read the files into memory
                memFishnet = 'in_memory' + os.sep + os.path.basename(intersectedFishnet) + "_view"
                try:
                    arcpy.CopyRows_management(intersectTable,memFishnet)
                except:
                ...
                variables = arcpy.ListFields(estimatesFile,"*WOMCHILD*DSM")
                for variable in variables:
                    name = variable.name
                    newField =  name.replace("DSM","CNTM")
                    # add the field
                    check = len(arcpy.ListFields(memFishnet,newField))
                    if check == 0:
