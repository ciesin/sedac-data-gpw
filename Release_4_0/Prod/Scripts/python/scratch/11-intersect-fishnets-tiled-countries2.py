...
def intersect(gdb):
    ...
    ###
    fcGDB = r'F:\gpw\pop_tables' + os.sep + iso + os.sep + iso + ".gdb"
    arcpy.env.workspace = fcGDB
    inFC = fcGDB + os.sep + str(arcpy.ListFeatureClasses("*gridding")[0])
    ######
    fishGDB = gdb
    ...
    prjFile = r'F:\gpw\custom_projections' + os.path.sep + rootName + "_mollweide.prj"
...
def main():
    ...
    workspaces = arcpy.ListWorkspaces("rus*","FOLDER")
    workspaces.sort()
    gdb_list = []
    for workspace in workspaces:
        ...
        if str(workDesc.workspaceType)=="FileSystem":
            ...
            arcpy.env.workspace = r'F:\gpw\fishnets'
            gdbs = arcpy.ListWorkspaces(os.path.basename(os.path.dirname(workspace))+"_orel*")
            for workspace in gdbs:
