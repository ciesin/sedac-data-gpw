...
# Modified by Kytt MacManus 9-6-12
# Import libraries
import arcpy
from arcpy import env

# define directories
workspace = r"\\Dataserver0\gpw\GPW4\Working\Census\ECU\staging\ecu-ingest.gdb"
...
tables=arcpy.ListTables("ecu_2010")
...
for table in tables:
    ...
    # Create empty list of newFields
    newFields=[]
    # Set i counter = len(fields)
    i = len(fields)
    # Create while loop to populate newFields list
    while i > 0:
        # Set j as suffix for variable creation
        j = i-1
        # Subtract 1 from counter
        i = i-1
        # Create newField Variables
        UTCID = "UTCID" + str(j)
        UCADMIN = "UCADMIN" + str(j)
        # Append newField Variables to newFields List
        newFields.append(UTCID)
        newFields.append(UCADMIN)        
