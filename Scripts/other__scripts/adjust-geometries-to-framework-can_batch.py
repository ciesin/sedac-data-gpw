...
# Define workspace
workspace = r'\\Dataserver0\gpw\GPW4\Preprocessing\Country\CAN\Ingest\Boundary\province_process'
env.workspace = workspace

# List Geodatabases
gdbs = arcpy.ListWorkspaces("Nunavut*")
...
for gdb in gdbs:
    ...
    frameworkBoundaries = name + "_gadm"
    ...
    if arcpy.Exists(inputBoundaries + "_adjusted"):
        pass
    else:
        ...
        if str(inputCount) <> str(clipCount):
            ...
            print "There are " + str(inputCount) + " input features, but only " + str(clipCount) + " clip features...CHECK"
            symBoundaries = clipBoundaries + "_symdiffCHECK"
        else:        
