...
root =r'\\Dataserver0\gpw\GPW4\Gridding\country\inputs\bra_state\preprocess'
...
for gdb in gdbs:
    ...
    fishnet = gdb + os.sep + iso + "_fishnet_clipped_intersect"
    if not arcpy.Exists(fishnet):
        ...
        if not int(arcpy.GetCount_management(fishnetLyr)[0])>0:
            ...
            try:
                arcpy.CalculateField_management(fishnetLyr,"AREAKMMASKED",0,"PYTHON")
