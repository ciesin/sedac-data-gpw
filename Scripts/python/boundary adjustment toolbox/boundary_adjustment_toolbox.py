...
frameworkBoundaries =os.path.join(in_memory, "frameworkBoundaries")
...
if __name__ == '__main__':
    ...
    for iso in ISO_list:
        ...
        if iso == "and":
            frameworkBoundaries_path = os.path.join(framework_gdb, iso.upper()+"_")
        else:
            frameworkBoundaries_path = os.path.join(framework_gdb, iso.upper())
        ...
        if not arcpy.Exists(frameworkBoundaries_path):
            arcpy.AddError(f'{frameworkBoundaries_path} does not exist.')
        ...
        try:
            ...
            # create a memory copy of input feature class
            arcpy.conversion.FeatureClassToFeatureClass(frameworkBoundaries_path, in_memory, 'frameworkBoundaries')
            arcpy.analysis.PairwiseDissolve(frameworkBoundaries_path, frameworkBoundaries, multi_part="SINGLE_PART")
        except arcpy.ExecuteError:
            arcpy.GetMessages()
        ...
        try:
            ...
            arcpy.management.CalculateGeometryAttributes(
                ...
                geometry_property=f"{finalArea} AREA_GEODESIC",
                length_unit="",
        ...
        if len(subBoundaries)>0:
            ...
            desc = arcpy.Describe(inputBoundaries)
