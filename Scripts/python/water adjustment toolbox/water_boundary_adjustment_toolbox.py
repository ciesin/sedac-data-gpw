...
def update_guid_based_on_water_type(feature_class):
    """
    Update the GUID field based on the WATER_TYPE field.
    Append '_IW' for Inland Water and '_ICE' for Ice.
    """
    with arcpy.da.UpdateCursor(feature_class, [GUID, WATER_TYPE]) as cursor:
        for row in cursor:
            if row[1] == 'IW':
                row[0] = f"{row[0]}_IW"
            elif row[1] == 'ICE':
    ...
    for iso in ISO_list:
        ...
        if args.gpw_project == 'gpwv4':
            ...
            if check_for_field(inputBName, GUID) == 0 :
                arcpy.AddField_management(inputBName, GUID,'TEXT')
                try:
                ...
                # Temporary field name (ensure it's unique and doesn't conflict with existing fields)
                temp_field = "TempTextField"

                # Step 1: Add a temporary text field
                arcpy.AddField_management(inputBName, temp_field, "TEXT")  # Adjust field_length if needed

                # Step 2: Calculate the temporary text field with values from the GUID field
                arcpy.CalculateField_management(inputBName, temp_field, f"!{GUID}!", "PYTHON3")

                # Step 3: Delete the original GUID field
                arcpy.DeleteField_management(inputBName, GUID)

                # Step 4: Rename the temporary text field to the original field name
                arcpy.AlterField_management(inputBName, temp_field, GUID, GUID)
        ...
        country_water_fc = f'Z:\GPW\GPW5\Preprocessing\Global\Water\country\{iso.upper()}.gdb\{iso}_water'
        if arcpy.Exists(country_water_fc):
            arcpy.AddMessage(f"The water feature class for '{iso}' exists.")
            try:
                arcpy.env.preserveGlobalIds = True
                arcpy.analysis.Identity(inputBoundaries, country_water_fc, identityBoundaries)
                arcpy.AddMessage(f"Created {identityBoundaries}")
            except arcpy.ExecuteError:
                arcpy.GetMessages(f'Dissolved to {finalBoundaries} failed.')
            except Exception as e:
                # Catch unexpected errors (e.g., variable name typo or file system error).
                print(f"An unexpected error occurred: {e}")
            
            
        
        else:
            arcpy.AddMessage(f"The water feature class for '{iso}' does not exist. Using global layer.")
            try:
                arcpy.env.preserveGlobalIds = True
        ...
        # Clean up cursor
        # del cursor
        arcpy.AddMessage(f"Success update cursor on {identityBoundaries}")
        

        try:
            arcpy.env.workspace = identityBoundaries
            ...
            arcpy.GetMessages(f'Dissolved and {finalBoundaries} created.')
        ...
        try:
            arcpy.management.CalculateGeometryAttributes(
                in_features=finalBoundaries,
                geometry_property=f"{IDENTITYAREAKM} AREA_GEODESIC",
                length_unit="",
        ...
        # Update GUID based on WATER_TYPE
        update_guid_based_on_water_type(finalBoundaries)
        ...
        if len(subBoundaries)>0:
            ...
            for row in subBoundaries:
                ...
                water_type_L.plot(ax=axes[0], edgecolor='red', linewidth=0, linestyle=linestyles['loosely dashed'], label="Land",  facecolor=(0.8, 0.6, 0.4, 0.65), legend=True)
                water_type_IW.plot(ax=axes[0], edgecolor='blue', linewidth=0, linestyle=linestyles['loosely dashed'],  label="Inland Water",  facecolor=(0.6, 0.7, 1, 0.65), legend=True)
                water_type_ICE.plot(ax=axes[0], edgecolor='blue', linewidth=0, linestyle=linestyles['loosely dashed'],  label="Ice",  facecolor=(0.2, 0.5, 0.8, 0.65), legend=True)
                ...
                water_type_L.plot(ax=axes[1], edgecolor='red', linewidth=1, linestyle=linestyles['loosely dashed'], label="Land",  facecolor="None", legend=True)
                water_type_IW.plot(ax=axes[1], edgecolor=(0.6, 0.7, 1), linewidth=1, linestyle=linestyles['loosely dashed'],  label="Inland Water",  facecolor="None", legend=True)
                water_type_ICE.plot(ax=axes[1], edgecolor=(0.2, 0.5, 0.8), linewidth=1, linestyle=linestyles['loosely dashed'],  label="Ice",  facecolor="None", legend=True)
                ...
                custom_patches = [Patch(edgecolor='black',  facecolor="None"),
                                Patch(edgecolor='red', facecolor=(0.8, 0.6, 0.4)),
                                Patch(edgecolor=(0.6, 0.7, 1), facecolor=(0.6, 0.7, 1)),
                                Patch(edgecolor=(0.2, 0.5, 0.8), facecolor=(0.2, 0.5, 0.8))] # Patch(edgecolor='black',  facecolor=(0, 0, 0, 0.5))]
