...
def transferData(inTable, schemaTable, csvFile):
    csvFile = csv.writer(open(csvFile,'wb'))
    yearFail = 0
    # grab iso code
    iso = inTable[:3]
    ...
    with arcpy.da.SearchCursor(inTable, cursorFields) as cursor:
        for cursorRow in cursor:
            ...
            for csvValue in csvValues:
                index = csvValues.index(csvValue)                        
                VARIABLE = header[index]
                csvFile.writerow((iso,VARIABLE,csvValue))
...
def main():
    ...
    # list tables
    tables = arcpy.ListTables("a*")
    tables.sort()
    # define agrTable
    agrTable = r'\\Dataserver0\gpw\GPW4\Gridding\global\inputs\agr_csvs\all_agr.csv'
    # open csv file and write header
    csvFile = csv.writer(open(agrTable,'wb'))
    csvFile.writerow(("ISO","VARIABLE","VALUE"))
    for inTable in tables:
        processTime = datetime.datetime.now()
        print "Processing " + inTable[:3]
        country = inTable[:3]
        # execute function
        transferData(inTable,inSchema,agrTable)
##        try:
