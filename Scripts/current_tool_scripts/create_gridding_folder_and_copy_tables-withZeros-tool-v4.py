...
## EDIT 11-20-13 - Kytt
## No need to make the GDB in a scratch directory and
## then copy onto the share, can just make it on the
## share itself to begin with.
...
if arcpy.Exists(isoGDB):
    arcpy.AddMessage("Geodatabase already exists, make sure you want to replace it...if so then manually delete and run this tool again!")
...
if lookupTableExist == "true":
    ...
    try:
       ...
       arcpy.AddMessage("Join was not successful.  Do UBID and USCID exist in your lookup table?")
       sys.exit("Join was not successful.  Do UBID and USCID  exist in your lookup table?")
else:
