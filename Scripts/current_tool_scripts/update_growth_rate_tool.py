...
if not arcpy.Exists(grTable):
    arcpy.AddMessage("The growth rate table is missing, please check")
    sys.exit("The growth rate table is missing, please check")
# define grJoin
grJoin = arcpy.GetParameterAsText(2)
##grJoin = "USCID"
# error catch
joinTest = arcpy.ListFields(inputTable,grJoin)
if not len(joinTest)==1:
    arcpy.AddMessage("The field you selected to join on is missing from the Census table, please check")
    sys.exit("The field you selected to join on is missing from the Census table, please check")
