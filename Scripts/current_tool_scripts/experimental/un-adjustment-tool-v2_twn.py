...
# 2-1213
#
# slight change in script needed to properly lookup Taiwan, which is without an
# ISO code in the UN adjustment table
# John Squires 06192014
...
grCalc = arcpy.GetParameterAsText(0)
# define input table
...
try:
    ...
    ##CHANGED ISO to UCID0 and hardcoded value for Taiwan
    arcpy.TableSelect_analysis(inUNTable,unTable,'"UCID0_numeric" = 158')
    arcpy.AddMessage("Created " + unTable)
except:
    arcpy.AddMessage("Did not create " + unTable)
    arcpy.GetMessages(2)
