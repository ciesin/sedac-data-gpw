...
def grid(outWS,csvFile):
    ...
    for field in gridFields:
        ...
        # Grid
        outGrid =  outputFolder + os.path.sep + gridName + "_TOTAL"
        print outGrid
        if arcpy.Exists(outGrid):
            arcpy.AddMessage(outGrid + " exists")
            print outGrid + " exists"
        else:
            # Isolate WATER CELLS > 0 AND MASKED CELLS = 0
            waterLyr = fcString + "_" + gridName + "_lyr"
            try:
                arcpy.MakeFeatureLayer_management(fish,waterLyr,""""SUM_WATERAREAKM" >0 AND "SUM_AREAKMMASKED" =0""")
            except:
                arcpy.GetMessages()
...
##    workspace = r'\\dataserver0\gpw\GPW4\Gridding\country\inputs\usa_state\test'
    ...
    # define csv file
    attributes = r'\\dataserver0\gpw\GPW4\Gridding\validation' + os.sep + "water_area_10_31_14.csv"
