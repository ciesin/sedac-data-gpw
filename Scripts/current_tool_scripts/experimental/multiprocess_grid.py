...
def grid(outWS):
    ...
    for field in gridFields:
        ...
        if arcpy.Exists(outAreaGrid):
            ...
            try:
                arcpy.PolygonToRaster_conversion(fish,"SUM_AREAKM",outAreaGrid,'CELL_CENTER','#',cellSize)
...
##    workspace = r'\\dataserver0\gpw\GPW4\Gridding\country\inputs\usa_state\test'
    ...
    # list gdbs
    gdbs = arcpy.ListWorkspaces("ABW*","FILEGDB")
####    gdbList = []
####    for gdb in gdbs:
...
####            print gdb
####            grid(gdb)
##    pool = multiprocessing.Pool(processes=44)#,maxtasksperchild=1)
##    pool.map(grid, gdbs) 
