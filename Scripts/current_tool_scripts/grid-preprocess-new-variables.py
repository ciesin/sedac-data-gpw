...
# define inputs
##inFC = r'\\Dataserver0\gpw\GPW4\Gridding\country\inputs\blz.gdb\blz_admin1_boundaries_2010'
...
##workspace = r'\\Dataserver0\gpw\GPW4\Gridding\country\inputs\blz.gdb'
...
if not arcpy.Exists(inFCG):
    ...
    try:
        ...
        arcpy.GetMessages()
# if inFCG exists then skip the area calculations
else:
    arcpy.AddMessage(inFCG + " already exists")
    print inFCG + " already exists"
    adminArea = "ADMINAREAKM"
    adminWaterArea = "ADMINWATERAREAKM"
    maskedArea = "ADMINAREAKMMASKED"
    featureArea = "AREAKM"
    ...
    whereCls = adminArea + ' > 0'
    tblSel = arcpy.MakeTableView_management(inFCG,tblSel,whereCls)
    # Create list of the new dsFields
    dsFieldList = []
    for field in fieldList:
        # define density field
        dsField = field + "_DS"
        # define masked density field
        maskedDSField = field + "_DSM"
        dsFieldList.append(dsField)
        dsFieldList.append(maskedDSField)
        # add density field and masked density field
        # Check to see if field exists
        dsFieldCheck = arcpy.ListFields(inFCG,dsField)
        if len(dsFieldCheck) == 1:
            arcpy.DeleteField_management(inFCG,dsField)
        # Check to see if field exists
        dsmFieldCheck = arcpy.ListFields(inFCG, maskedDSField)
        if len(dsmFieldCheck) == 1:
            arcpy.DeleteField_management(inFCG,maskedDSField)
        try:
            arcpy.AddField_management(inFCG,dsField,'DOUBLE')
            arcpy.AddField_management(inFCG,maskedDSField,'DOUBLE')
        except:
...
##            print "Calculated " + maskedDSField
    ...
    # define intersected fishnet and inFCG
    clipnetInt = clipnet + "_intersect"

    # add dsFields
    for joinVariable in dsFieldList:
        joinFieldCheck = arcpy.ListFields(clipnetInt,joinVariable)
        if not len(joinFieldCheck)==1:
            arcpy.AddField_management(clipnetInt,joinVariable,'Double')
...
##            arcpy.AddMessage(datetime.datetime.now() - addTime)
    ...
    try:
        ...
        print arcpy.GetMessages()
    print "Joined Density Fields in " + str(datetime.datetime.now() - joinTime)
    arcpy.AddMessage("Joined Density Fields in " + str(datetime.datetime.now() - joinTime)) 
    ...
    for field in fieldList:
        ...
        joinCNTFields.append("SUM_" + maskedCNTField)    
#######
        # Check to see if cntField exists
        cntFieldCheck = arcpy.ListFields(clipnetInt,cntField)
        if len(cntFieldCheck) == 1:
            arcpy.DeleteField_management(clipnetInt,cntField)
            try:
                arcpy.DeleteField_management(inFish,"SUM_" + cntField)
            except:
                pass
        maskedCNTFieldCheck = arcpy.ListFields(clipnetInt,maskedCNTField)
        if len(maskedCNTField) == 1:
            arcpy.DeleteField_management(clipnetInt,maskedCNTField)
            try:
                arcpy.DeleteField_management(inFish,maskedCNTField)
            except:
                pass
    ...
    sumTbl = inFC + "_aggregated_estimates"
    pixelID = "PIXELID"
    # remove existing sumTbl
    if arcpy.Exists(sumTbl):
        arcpy.Delete_management(sumTbl)        
    try:
        arcpy.Statistics_analysis(clipnetInt,sumTbl,cntFields,pixelID)
        arcpy.AddIndex_management(sumTbl,pixelID,pixelID + "_index","UNIQUE")    
        arcpy.AddMessage("Calculated Statistics")
...
##    # join results to the original fishnet
    ...
    try:
        ...
        arcpy.GetMessages()
    # delete fields
    delFields = arcpy.ListFields(inFish,"*_1")
    for delField in delFields:
        delField = delField.name
        try:
            arcpy.DeleteField_management(inFish,delField)
        except:
            print arcpy.GetMessages()
