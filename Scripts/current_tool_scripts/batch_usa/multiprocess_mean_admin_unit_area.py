...
def meanArea(gdb):
    ...
    # define outputs
    outGDB = gdb.replace("state" + os.sep + "inputs","state" + os.sep + "rasters")[:-4] + "_grids.gdb"
    outInputs = outGDB + os.path.sep + iso.upper() + "_NUMINPUTS" 
    meanUnits = outGDB + os.path.sep + iso.upper() + "_MEAN_ADMIN_UNIT_AREA"
    scratchGDB = r'\\dataserver0\gpw\GPW4\Gridding\country\inputs\usa_state\scratch' + os.sep + iso + ".gdb"
    # check if scratchGDB exists, if not then create it
    if not arcpy.Exists(scratchGDB):
        try:
            arcpy.CreateFileGDB_management(r'\\dataserver0\gpw\GPW4\Gridding\country\inputs\usa_state\scratch',iso)
...
##            meanArea(gdb)
        numProc = len(gdbs)
        print "The number of processes = " + str(numProc)
        ...
        pool.close()
        pool.join()
        print wildcard + " wildcard executed in: "
