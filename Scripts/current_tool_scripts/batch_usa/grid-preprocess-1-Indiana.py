...
## IMPORTANT
## MODIFICATIONS MADE TO INLUCDE FIELD IN FIELDLIST THAT IS UNIQUE TO US CASE
...
def preprocess1(outWS):
    # define inputs
##    inFC = r'D:\GPW\mar.gdb\mar_admin3_boundaries_2010'
    inFC = outWS + os.sep + os.path.basename(outWS)[:-4]
...
##    workspace = r'D:\GPW\mar.gdb'
    ...
    if not arcpy.Exists(inFish):
        ...
        sys.exit("The input fishnet does not exist, check the geodatabase " + inFish)
...
##        arcpy.CalculateField_management(adminMaskedLYR,maskedArea,0, "PYTHON")
    ...
    if grCalc == "true":
        fieldList = ["ATOTPOPBT_ADMIN2"]
        try:
##            arcpy.AddField_management(inFCG,"ATOTPOPBT_ADMIN2",'DOUBLE')
            arcpy.AddMessage("Added " + "ATOTPOPBT_ADMIN2")
        except:
            arcpy.GetMessages()
...
def main():
    ...
    # define workspace
    workspace = r'\\Dataserver0\gpw\GPW4\Gridding\country\inputs\usa_state\temp'
